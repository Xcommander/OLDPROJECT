...
###############################   用户参数设定区 起始  ############################################
CODE_PATH="repo init -u  git@10.0.30.7:GINR6753_65C_L1_SW1/tools/manifest.git -b master"
BUILD_PROJECT=E169F_TW
PRODUCT_NAME=full_E169F
BUILD_PROJECT_NAME=E169F
IN_VERSION=TW_P635F32V1.0.0B08
OUT_VERSION=BLADE_D2_TWV1.0.0B01
#PROVINCE_INFO=
#OPERATOR_INFO=
INCREMENTAL_VERSION=
CORE=24
...
RELEASE_PROJECT=E169F
NEED_GMS=yes          # yes or no
VERSION_FILE=versionTW
...
function download_code()
{
    ...
    if [ $NEED_GMS = "yes" ] ; then
        ...
        {
            git clone git3@10.0.30.6:external/google/GMS_Above5.0.git
        }
        fi
        cd $path
        echo "GMS update -e"
...
function build_diff()
{
    ...
    if [ $FOTA_STYLE = "FOTA_LEWA" ] ; then
        ...
        if [ $update_exist = "1" ]; then
        {
            ...
            if [ $update_a_exist = "1" ]; then
				...
				./build/tools/releasetools/ota_from_target_files   -n  -i update_a.zip update_b.zip updateA2B.zip
                if [ -f ./updateA2B.zip ]; then
                    a2b_success=1
                fi
            fi
            ...
            if [ $update_c_exist = "1" ]; then
				...
				./build/tools/releasetools/ota_from_target_files   -n  -i update_b.zip update_c.zip updateB2C.zip
                if [ -f ./updateB2C.zip ]; then
                    b2c_success=1
                fi
            fi
 ...
 ##               source change_java.sh 1.6
                ...
                if [ $KK_CODE_SIGN_FOTA = "yes" ] ; then
                    ./build/tools/releasetools/ota_from_target_files   -n  --block  -i update_a.zip update_b.zip unsign_updateA2B.zip
                    ...
                    ./build/tools/releasetools/ota_from_target_files   -n  --block  -i update_a.zip update_b.zip updateA2B.zip
                fi
...
##                source change_java.sh 1.6
                ...
                if [ $KK_CODE_SIGN_FOTA = "yes" ] ; then
                    ./build/tools/releasetools/ota_from_target_files   -n  --block  -i update_b.zip update_c.zip unsign_updateB2C.zip
                    ...
                    ./build/tools/releasetools/ota_from_target_files   -n  --block  -i update_b.zip update_c.zip updateB2C.zip
                fi
...
function copy_package()
{
    ...
    if [ $FOTA_STYLE = "FOTA_LEWA" ] ; then
        ...
        if [ -f ./updateA2*.zip ]; then
            cp ./updateA2*.zip ./out_package/
        fi
        
        if [ -f ./updateB2C.zip ]; then
            cp ./updateB2C.zip ./out_package/
        fi
        
        if [ -f $TARGET_FILES_PATH/$PRODUCT_NAME-target_files-*.zip ]; then
            cp $TARGET_FILES_PATH/$PRODUCT_NAME-target_files-*.zip ./out_package/
        fi
    ...
    cd $BUILD_PATH/obj/KERNEL_OBJ
    zip -rq vmlinux.zip vmlinux
    cd -
    mv $BUILD_PATH/obj/KERNEL_OBJ/vmlinux.zip ./out_package/

    if [ -f $BUILD_PATH/$PRODUCT_NAME-ota-*.zip ]; then
        cp $BUILD_PATH/$PRODUCT_NAME-ota-*.zip ./out_package/
    fi
